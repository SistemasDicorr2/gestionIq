...
<template>
  <PopoverRoot :open="show" @update:open="(val) => !val && $emit('close')">
    <PopoverPortal>
      <PopoverContent
        side="bottom"
        :side-offset="8"
        align="end"
        class="w-80 sm:w-96 rounded-2xl bg-white dark:bg-slate-900 shadow-2xl border border-slate-200/80 dark:border-slate-800 focus:outline-none z-50 flex flex-col overflow-hidden animate-in fade-in-0 zoom-in-95"
        role="dialog"
        ...
        <div class="flex-grow overflow-y-auto max-h-[60vh] p-1.5 space-y-1 custom-scrollbar">
          ...
          <ul v-else class="space-y-1" role="list">
            <li
              ...
              class="group p-3 rounded-xl transition-all duration-150 cursor-pointer flex items-start gap-3 border border-transparent"
              :class="[
...
// Reka UI Popover
import { PopoverRoot, PopoverContent, PopoverPortal } from 'reka-ui';
